...
def estimate_coeff_mat_batch_max_iter(target_embeddings, basis_pred, device):
    ...
    for i in range(n_not_sparse):
        ...
        coeff_mat_trans_temp = torch.zeros(batch_size, basis_pred.size(1), target_embeddings.size(1),
...
def estimate_coeff_mat_batch(target_embeddings, basis_pred, L1_losss_B, device, max_iter=100):
    ...
    coeff_mat_prev = torch.randn(batch_size, target_embeddings.size(1), basis_pred.size(1), requires_grad=False,
                                 device=device)
...
def target_emb_preparation(target_index, w_embeddings, n_batch, n_set, rotate_shift):
    ...
    target_embeddings = target_embeddings / (0.000000000001 + target_embeddings.norm(dim=2,
                                                                                     keepdim=True))  # If this step is really slow, consider to do normalization before doing unfold

    # target_embeddings_4d = target_embeddings.view(-1,n_batch, n_set, target_embeddings.size(2))
    target_embeddings_rotate = torch.cat(
        (target_embeddings[rotate_shift:, :, :], target_embeddings[:rotate_shift, :, :]), dim=0)
...
def compute_loss_set(output_emb, w_embeddings, target_set, L1_losss_B, device, w_freq, coeff_opt, compute_target_grad,
                     coeff_opt_algo):
    ...
    if compute_target_grad:
        target_embeddings, target_emb_neg = target_emb_preparation(target_set, w_embeddings, n_batch, n_set,
                                                                   rotate_shift)
    else:
        with torch.no_grad():
            target_embeddings, target_emb_neg = target_emb_preparation(target_set, w_embeddings, n_batch, n_set,
    ...
    with torch.no_grad():
        ...
        target_freq_inv_norm_neg = torch.cat(
            (target_freq_inv_norm[rotate_shift:, :], target_freq_inv_norm[:rotate_shift, :]), dim=0)

        # coeff_mat = estimate_coeff_mat_batch(target_embeddings.cpu(), basis_pred.detach(), L1_losss_B)
        if coeff_opt == 'lc':
            if coeff_opt_algo == 'sgd_bmm':
                coeff_mat = estimate_coeff_mat_batch(target_embeddings.detach(), basis_pred.detach(), L1_losss_B,
                                                     device)
                coeff_mat_neg = estimate_coeff_mat_batch(target_emb_neg.detach(), basis_pred.detach(), L1_losss_B,
                                                         device)
            else:
                lr_coeff = 0.05
                iter_coeff = 60
                with torch.enable_grad():
                    coeff_mat = estimate_coeff_mat_batch_opt(target_embeddings.detach(), basis_pred.detach(),
                                                             L1_losss_B, device, coeff_opt_algo, lr_coeff, iter_coeff)
                    coeff_mat_neg = estimate_coeff_mat_batch_opt(target_emb_neg.detach(), basis_pred.detach(),
                                                                 L1_losss_B, device, coeff_opt_algo, lr_coeff,
                                                                 iter_coeff)
        else:
    ...
    loss_set_neg = - torch.mean(
        target_freq_inv_norm_neg * torch.pow(torch.norm(pred_embeddings_neg - target_emb_neg, dim=2), 2))
    loss_coeff_pred = torch.mean(
        torch.pow(coeff_sum_basis / coeff_mean - coeff_pred[:, :, 0].view_as(coeff_sum_basis), 2))
    loss_coeff_pred += torch.mean(
        torch.pow(coeff_sum_basis_neg / coeff_mean - coeff_pred[:, :, 1].view_as(coeff_sum_basis_neg), 2))
