...
from utils import seed_all_randomness, load_word_dict, load_emb_from_path, load_idx2word_freq, loading_all_models, \
    load_testing_article_summ, str2bool, Logger
...
if 'ours' in args.method_set:
    parallel_encoder, parallel_decoder, encoder, decoder, word_norm_emb = loading_all_models(args, idx2word_freq,
                                                                                             device, args.max_sent_len)
...
all_clustering_models = [KMeans(n_clusters=i, max_iter=100, n_init=1, precompute_distances=True, n_jobs=5) for i in
                         range(1, args.top_k_max + 1)]
...
def article_to_embs_sentvec(article, word_d2_idx_freq, device):
    ...
    for i_sent, sent in enumerate(article):
        ...
        sent_embs_tensor[i_sent, :] = np_array_to_tensor(bio_s2v.embed_sentence(sent_proc), device)
        w_emb_list = []
        # print(sent)
    ...
    for i_w, (w, w_freq) in enumerate(artical_counter.items()):
        ...
        if w in word_d2_idx_freq:
            w_idx, freq, freq_prob = word_d2_idx_freq[w]
            freq_prob_tensor[i_w] = freq_prob
            all_words_tensor[i_w, :] = np_array_to_tensor(bio_s2v.embed_unigrams([w]), device)
...
def rank_sents(basis_coeff_list, article, citations, word_norm_emb, word_d2_idx_freq, top_k_max, device):
    ...
    if args.method_set != 'bert':
        sent_embs_tensor, sent_embs_w_tensor, all_words_tensor, w_emb_tensors_list, sent_lens, freq_prob_tensor, w_freq_tensor = article_to_embs(
            article, word_norm_emb, word_d2_idx_freq, device)
        cit_sent_embs_tensor, cit_sent_embs_w_tensor, cit_all_words_tensor, cit_w_emb_tensors_list, cit_sent_lens, cit_freq_prob_tensor, cit_w_freq_tensor = article_to_embs(
            citations, word_norm_emb, word_d2_idx_freq, device)
        freq_w_4_tensor = alpha / (alpha + freq_prob_tensor)
        ## TODO: do it twice, one for article and one for citations
    if 'bert' in args.method_set:
        sent_embs_tensor_bert, all_words_tensor_bert, w_emb_tensors_list_bert, sent_lens_bert, freq_prob_tensor_bert, w_freq_tensor_bert = article_to_embs_bert(
            article, bert_tokenizer, bert_model, word_d2_idx_freq, device)
        cit_sent_embs_tensor_bert, cit_all_words_tensor_bert, cit_w_emb_tensors_list_bert, cit_sent_lens_bert, cit_freq_prob_tensor_bert, cit_w_freq_tensor_bert = article_to_embs_bert(
            citations, bert_tokenizer, bert_model, word_d2_idx_freq, device)
        ## TODO: same as above
    ...
    if 'embs' in args.method_set:
        m_d2_sent_ranks['sent_emb_dist_avg'] = select_by_avg_dist_boost(cit_sent_embs_tensor, all_words_tensor,
                                                                        w_freq_tensor, top_k_max, device)
        m_d2_sent_ranks['sent_emb_dist_avg_freq_4'] = select_by_avg_dist_boost(cit_sent_embs_tensor, all_words_tensor,
                                                                               w_freq_tensor, top_k_max, device,
                                                                               freq_w_4_tensor)
        m_d2_sent_ranks['sent_emb_freq_4_dist_avg_freq_4'] = select_by_avg_dist_boost(cit_sent_embs_w_tensor,
                                                                                      all_words_tensor, w_freq_tensor,
                                                                                      top_k_max, device,
                                                                                      freq_w_4_tensor)
        m_d2_sent_ranks['norm_w_in_sent'] = select_by_topics(cit_w_emb_tensors_list, all_words_tensor, w_freq_tensor,
                                                             top_k_max, device, cit_sent_lens)
        m_d2_sent_ranks['norm_w_in_sent_freq_4'] = select_by_topics(cit_w_emb_tensors_list, all_words_tensor,
                                                                    w_freq_tensor, top_k_max, device, cit_sent_lens,
                                                                    freq_w_tensor=freq_w_4_tensor)
    if 'cluster' in args.method_set:
        ...
        m_d2_sent_ranks['sent_emb_cluster_word'] = select_by_clustering_words(sent_embs_tensor, all_words_tensor,
                                                                              top_k_max, device, w_freq_tensor)
        m_d2_sent_ranks['sent_emb_cluster_word_freq_4'] = select_by_clustering_words(sent_embs_tensor, all_words_tensor,
                                                                                     top_k_max, device,
                                                                                     w_freq_tensor * freq_w_4_tensor)
        m_d2_sent_ranks['sent_emb_freq_4_cluster_sent'] = select_by_clustering_words(sent_embs_w_tensor,
                                                                                     sent_embs_w_tensor, top_k_max,
                                                                                     device)
        m_d2_sent_ranks['sent_emb_freq_4_cluster_sent_len'] = select_by_clustering_words(sent_embs_w_tensor,
                                                                                         sent_embs_w_tensor, top_k_max,
                                                                                         device, cit_sent_lens)
    ...
    with open(args.input + '/' + file_name) as f_in:
        ...
        with torch.no_grad():
            ...
            if 'ours' in args.method_set:
                ...
                dataloader_test = load_testing_article_summ(word_d2_idx_freq, article, args.max_sent_len,
                                                            args.batch_size, device)
                cit_dataloader_test = load_testing_article_summ(word_d2_idx_freq, citations, args.max_sent_len,
                                                                args.batch_size, device)
                ...
                basis_coeff_list, article_proc = utils_testing.output_sent_basis_summ(dataloader_test, parallel_encoder,
                                                                                      parallel_decoder, args.n_basis,
                                                                                      idx2word_freq)
                cit_basis_coeff_list, citation_proc = utils_testing.output_sent_basis_summ(cit_dataloader_test,
                                                                                           parallel_encoder,
                                                                                           parallel_decoder,
                                                                                           args.n_basis, idx2word_freq)
            else:
                cit_basis_coeff_list = None
                # article_proc = article
                citation_proc = citations
            fname_d2_sent_rank[file_name] = rank_sents(cit_basis_coeff_list, article_proc, citation_proc, word_norm_emb,
                                                       word_d2_idx_freq, args.top_k_max, device)
...
if 'cluster' in args.method_set:
    all_method_list += ['sent_emb_freq_4_cluster_sent', 'sent_emb_freq_4_cluster_sent_len', 'sent_emb_cluster_word',
                        'sent_emb_cluster_word_freq_4']
...
not_inclusive_methods = set(
    ['sent_emb_freq_4_cluster_sent', 'sent_emb_freq_4_cluster_sent_len', 'sent_emb_freq_4_cluster_word',
     'sent_emb_freq_4_cluster_word_freq_4', 'sent_emb_cluster_sent', 'sent_emb_cluster_sent_len',
